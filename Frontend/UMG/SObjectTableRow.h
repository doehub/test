...
public:
	...
	void Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime ) override
	{
		...
		if ( bShowSelection != bSelected )
		{
			...
			IKFListItem::Execute_OnItemSelected( WidgetObject, bSelected );
		}
	}
	...
	{
		ListIndex = InIndexInList;
		IKFListItem::Execute_OnItemIndex( WidgetObject, InIndexInList );
	}

	/** @return true if the corresponding item is expanded; false otherwise*/
	bool IsItemExpanded() const override
	{
//		return IKFListItem::Execute_IsItemExpanded( WidgetObject );
		return false;
	}
...
//		return IKFListItem::Execute_GetIndentLevel( WidgetObject );
	...
	{
//		return IKFListItem::Execute_DoesItemHaveChildren( WidgetObject );
		return INDEX_NONE;
	}
	...
	{
//		IKFListItem::Execute_Private_OnExpanderArrowShiftClicked( WidgetObject );
	}

	/** @return the size for the specified column name */
	virtual FVector2D GetRowSizeForColumn(const FName& InColumnName) const
	{
		return FVector2D();
	}
